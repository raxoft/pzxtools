...
void wav_open( FILE * file )
{
    ...
    // Make sure the file starts with a WAV header.

    wav_header( NULL, 0 ) ;
}

/**
 * Commit any buffered WAV output to WAV output file and stop using that file.
 */
void wav_close( void )
{
    hope( output_file ) ;
...
/**
 * Write given memory block of given size to output file as WAV block with given tag.
 */
void wav_write_block( const uint tag, const void * const data, const uint size )
{
    ...
    wav_write( header, sizeof( header ) ) ;
    wav_write( data, size ) ;
}

/**
 * Write content of given buffer to output file as WAV block with given tag.
 *
 * @note The buffer content is cleared afterwards, making it ready for reuse.
 */
void wav_write_buffer( const uint tag, Buffer & buffer )
{
    // Write entire buffer to the file.

    wav_write_block( tag, buffer.get_data(), buffer.get_data_size() ) ;
...
/**
 * Append given memory block of given size to WAV header block.
 */
void wav_header( const void * const data, const uint size )
{
    hope( data || size == 0 ) ;
    ...
    if ( header_buffer.is_empty() ) {
        header_buffer.write_little< u8 >( WAV_MAJOR ) ;
        header_buffer.write_little< u8 >( WAV_MINOR ) ;
    }
...
/**
 * Append given amount of characters from given string to WAV header block.
 */
void wav_info( const void * const string, const uint length )
{
    ...
    wav_header( string, length ) ;
}

/**
 * Append given null terminated string to WAV header block.
 */
void wav_info( const char * const string )
{
    hope( string ) ;
    wav_info( string, std::strlen( string ) ) ;
}

/**
 * Append given amount of pulses of given duration to WAV pulse block.
 *
 * @note The @a count and @a duration must fit in 15 and 31 bits, respectively.
 */
void wav_store( const uint count, const uint duration )
{
    hope( count > 0 ) ;
    hope( count < 0x8000 ) ;
...
/**
 * Append pulse of given duration to WAV pulse block.
 *
 * @note The @a duration must fit in 31 bits.
 */
void wav_pulse( const uint duration )
{
    hope( duration < 0x80000000 ) ;
    ...
    if ( pulse_count > 0 ) {
        ...
        wav_store( pulse_count, pulse_duration ) ;
    }
...
/**
 * Commit any buffered header and/or pulse output to the WAV output file.
 */
void wav_flush( void )
{
    ...
    if ( header_buffer.is_not_empty() ) {
        wav_write_buffer( WAV_HEADER, header_buffer ) ;
    }
    ...
    if ( pulse_buffer.is_not_empty() ) {
        wav_write_buffer( WAV_PULSES, pulse_buffer ) ;
    }
}

/**
 * Append given data block to WAV output file as WAV data block.
 */
void wav_data(
...
)
{
    ...
    wav_write_buffer( WAV_DATA, data_buffer ) ;
}

/**
 * Test if given pulse sequence matches pulses in given pulse stream.
 */
bool wav_matches( const word * const pulses, const word * const end, const word * const sequence, const uint count )
{
    hope( pulses ) ;
    hope( end ) ;
    hope( pulses <= end ) ;
...
/**
 * Try to pack given pulses to WAV data block using given pulse sequences.
 */
bool wav_pack(
...
)
{
    ...
    while ( data < end ) {
        ...
        if ( wav_matches( data, end, sequence_0, pulse_count_0 ) ) {
            value <<= 1 ;
            data += pulse_count_0 ;
        }
        else if ( wav_matches( data, end, sequence_1, pulse_count_1 ) ) {
            value <<= 1 ;
            value |= 1 ;
            data += pulse_count_1 ;
        }
...
)
{
    ...
    if ( order == 0 ) {
        return wav_pack( pulses, pulse_count, initial_level, pulse_count_0, pulse_count_1, sequence_0, sequence_1, tail_cycles ) ;
    }
    else {
        return wav_pack( pulses, pulse_count, initial_level, pulse_count_1, pulse_count_0, sequence_1, sequence_0, tail_cycles ) ;
    }
}

/**
 * Try to pack given pulses to WAV data block, guessing the pulse sequences automatically.
 */
bool wav_pack(
    const word * const pulses,
    const uint pulse_count,
...
)
{
    ...
    for ( uint pulse_count_0 = limit ; pulse_count_0 > 0 ; pulse_count_0-- ) {
        ...
        while ( wav_matches( sequence_1, end, sequence_0, pulse_count_0 ) ) {
        ...
        if ( sequence_1 == end ) {
            wav_pack( pulses, pulse_count, initial_level, pulse_count_0, 0, sequence_0, NULL, sequence_order, tail_cycles ) ;
            return true ;
        }
        ...
        for ( uint pulse_count_1 = limit ; pulse_count_1 > 0 ; pulse_count_1-- ) {
            if ( wav_pack( pulses, pulse_count, initial_level, pulse_count_0, pulse_count_1, sequence_0, sequence_1, sequence_order, tail_cycles ) ) {
                return true ;
            }
        }
    }
...
/**
 * Append WAV pause block of pause of given duration and level to WAV output file.
 *
 * @note The duration must fit in 31 bits.
 */
void wav_pause( const uint duration, const bool level )
{
    hope( duration < 0x80000000 ) ;

    wav_flush() ;
    data_buffer.write_little< u32 >( ( level << 31 ) | duration ) ;
    wav_write_buffer( WAV_PAUSE, data_buffer ) ;
}

/**
 * Append WAV stop block with given flags to WAV output file.
 */
void wav_stop( const uint flags )
{
    hope( flags <= 0xFFFF ) ;

    wav_flush() ;
    data_buffer.write_little< u16 >( flags ) ;
    wav_write_buffer( WAV_STOP, data_buffer ) ;
}

/**
 * Append WAV browse block using given amount of characters from given string to WAV output file.
 */
void wav_browse( const void * const string, const uint length )
{
    wav_flush() ;
    wav_write_block( WAV_BROWSE, string, length ) ;
}

/**
 * Append WAV browse block using given string to WAV output file.
 */
void wav_browse( const char * const string )
{
    hope( string ) ;
    wav_browse( string, std::strlen( string ) ) ;
}
